...
public class TemplateRBAction extends Action {
	VMTreeObjectTemplate vmObject;
	List<VMTreeObjectTemplate> vmList;
	VMTreeObjectSR srObject;
	VMTreeObjectPool poolObject;
	Connection connection;
	
	public TemplateRBAction(VMTreeObjectTemplate vmObject,VMTreeObjectSR srObject){
		super();
	...
	public TemplateRBAction(List<VMTreeObjectTemplate> vmList,VMTreeObjectSR srObject){
		super();
		setText(srObject.getName());	
		this.vmList = vmList;
		this.srObject=srObject;
...
//		}
	    ...
	    protected IStatus run(IProgressMonitor monitor) { 
	        ...
	        for(VMTreeObjectTemplate vmObject:vmList){
...
//		        	name = name +(index+1);
//		       // }
		        backupVM = new VMTreeObjectTemplate(name,connection
						,null,null);
				...
				try{
					...
					newVm.setIsATemplate(connection, true);
				...
				poolObject.templateMap.put(newVm, backupVM);
	...
	public void run(){
		if(vmList==null){
			vmList = new ArrayList<VMTreeObjectTemplate>();
			vmList.add(vmObject);
		}
		VMTreeView viewer = Constants.treeView;
		final VMEvent event=new VMEvent();
		event.setDatetime(new Date());
		event.setDescription("异地备份虚拟机 '" + vmObject.getName() + "'");
